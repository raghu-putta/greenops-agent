...
# Add Configure GCP button after Run Real GCP button
old = "class=\"btn btn-real\" id=\"btn-real\" onclick=\"run('real')\">&#9729; Run Real GCP</button>"
new = "class=\"btn btn-real\" id=\"btn-real\" onclick=\"run('real')\">&#9729; Run Real GCP</button>\n      <button class=\"btn-settings\" onclick=\"openSettings()\">&#9881; Configure GCP</button>"
if old in content:
    content = content.replace(old, new)
    print('OK - Configure GCP button added')
else:
    print('WARNING - button pattern not found, trying alternate...')
    old2 = "btn btn-real\" id=\"btn-real\" onclick=\"run('real')\">"
    if old2 in content:
        idx = content.find(old2)
        end = content.find('</button>', idx) + 9
        orig = content[idx-8:end]
        print('Found button:', orig[:80])

# Add settings panel HTML + JS before </body>
robot_panel = '''
...
# Add FastAPI test-connection endpoint
endpoint = '''
