...
async def run_pipeline(mode: str, user_cfg: dict = None):
    global _DEFAULT_ENV
    try:
        _DEFAULT_ENV
    except NameError:
...
('globeCanvas', {theme:'Aurora', spin:0.16, energy:'Overdrive'});
</script>
...
<div id="report-dl" style="display:none;padding:10px 16px;background:#161b22;border-top:2px solid #34d399;gap:8px;flex-wrap:wrap;align-items:center;margin-top:0;">
...
<script>
  ...
  // ── Run ─────────────────────────────────────────────────────────────────────
  function run(mode) {
    var _cfg={};try{_cfg=JSON.parse(sessionStorage.getItem('gops-cfg')||'{}');}catch(e){}
